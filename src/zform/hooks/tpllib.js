...
<slot-com :defs="slotContent" :attrs="{parts}"
           :binds="{key: '${key}', level: getIntVal([${level}]), partName: '${append.part.name}', config: getUI_CONFIG('${configPath}'), deep: getObjDeep([${pathArrStr.slice(1)}]), pathArr: [${pathArrStr.slice(1)}], configPath: '${configPath}', label: '${getLabel(append.CONFIG, configPath, key)}', selfpath: '${fromPath}',  process: '${append.CONFIG.process}', parts: parts, BASE_PATH:'${append.BASE_PATH}' }"
...
`;
      ...
      context.tpl = context.tpl + `
<slot-com :defs="slotContent" :attrs="{parts}"
           :binds="{key: '${key}', level: getIntVal([${level}]), partName: '${append.part.name}', configPath: '${configPath}', selfpath: '${fromPath}', deep: getObjDeep([${pathArrStr.slice(1)}]),  pathArr: [${pathArrStr.slice(1)}], process: '${append.CONFIG.process}', parts: parts, BASE_PATH:'${append.BASE_PATH}' }"
...
`;
      ...
      if (!p.hidden) {
        ...
        if (wrap_start_tag) {
          context.tpl = context.tpl + `<${wrap_start_tag} field-key="${key}"
:binds="{ key: '${key}',  label: '${getLabel(append.CONFIG, configPath, key)}',   config: getUI_CONFIG('${configPath}'),  selfpath: '${fromPath}',  partName: '${append.part.name}', pathArr: [${pathArrStr.slice(1)}] }" 
>`;
        }
